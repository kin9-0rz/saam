...
from apkutils.dex import dexparser
from apkutils.dex.jvm import writeclass
from apkutils.dex.mutf8 import decode
from apkutils.dex.jvm.optimization import options
...
def dex2jar(inputfile, force=False, output=None, fast=False):

    dexs = []
    if inputfile.lower().endswith('.apk'):
        with zipfile.ZipFile(inputfile, 'r') as z:
            for name in z.namelist():
                if name.startswith('classes') and name.endswith('.dex'):
                    dexs.append(z.read(name))
    else:
        dexs.append(read(inputfile))
    ...
    outname = output or inputfile.rpartition(
        '/')[-1].rpartition('.')[0] + '-enjarify.jar'
    try:
        outfile = open(outname, mode=('wb' if force else 'xb'))
...
def main():
    ...
    dex2jar(args.inputfile, args.force, args.output, args.fast)
    # dexs = []
